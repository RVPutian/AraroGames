const APPWRITE_ADMIN_CONFIG = {
    ...
    downloadSettingsTableId: 'download_settings',
...
if (showcaseForm) {
    const formActions = showcaseForm.querySelector('.form-actions') || showcaseForm;
    ...
    if (submitBtn && submitBtn.parentNode) {
        submitBtn.parentNode.insertBefore(showcaseCancelBtn, submitBtn);
    } else {
        showcaseForm.appendChild(showcaseCancelBtn);
    }
}
...
function resetShowcaseForm() {
    ...
    // Re-enable the required attribute for new cards
    if (showcaseImageInput) {
        showcaseImageInput.setAttribute('required', 'true');
    }
}
...
window.startEditingShowcase = function(rowId) {
    ...
    editingShowcaseImageId = card.imageId; 
    ...
    // Temporarily remove the 'required' attribute so you can submit without a new image
    if (showcaseImageInput) {
        showcaseImageInput.removeAttribute('required');
    }
...
if (showcaseForm) {
    showcaseForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // 1. Check if a NEW image was uploaded during the edit
            if (showcaseImageInput.files && showcaseImageInput.files.length > 0) {
                const file = showcaseImageInput.files[0];
                ...
                // Delete old image to keep your storage clean
                ...
                // Failsafe: Prevent creation of NEW cards without an image
            ...
            // 2. Route the save logic based on State
...
// ==========================================
let currentSettingsRowId = null; // We need to remember the Row ID to update it

document.addEventListener('DOMContentLoaded', async () => {
    // Wait a brief moment to ensure Appwrite is fully initialized
    setTimeout(loadDownloadSettings, 500);
});

async function loadDownloadSettings() {
    const downloadInput = document.getElementById('adminDownloadLink');
    const videoInput = document.getElementById('adminVideoLink');
    if (!downloadInput || !videoInput) return;

    try {
        const tablesDB = createTablesService();
        const response = await tablesDB.listRows({
            databaseId: APPWRITE_ADMIN_CONFIG.databaseId,
            tableId: APPWRITE_ADMIN_CONFIG.downloadSettingsTableId
        });

        // If a row exists, grab the data and populate the form
        if (response.rows && response.rows.length > 0) {
            const settings = response.rows[0];
            currentSettingsRowId = settings.$id; // Save the ID for later
            
            downloadInput.value = settings.downloadLink || '';
            videoInput.value = settings.videolink || '';
        }
    } catch (error) {
...
if (downloadSettingsForm) {
    downloadSettingsForm.addEventListener('submit', async (e) => {
        ...
        const submitBtn = downloadSettingsForm.querySelector('button[type="submit"]');
        submitBtn.disabled = true;
        submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Saving...';

        const downloadLink = document.getElementById('adminDownloadLink').value;
        const videoLink = document.getElementById('adminVideoLink').value;
        ...
        try {
            ...
            if (currentSettingsRowId) {
                ...
                // CREATE new row (Failsafe, just in case the row was deleted)
                const { ID } = window.Appwrite;
                await tablesDB.createRow({
                    databaseId: APPWRITE_ADMIN_CONFIG.databaseId,
                    tableId: APPWRITE_ADMIN_CONFIG.downloadSettingsTableId,
            ...
            showDashboardNotification('success', 'Download links updated successfully!');
            await loadDownloadSettings(); // Reload to ensure we have the latest ID

        } catch (error) {
            console.error('Failed to save settings:', error);
            showDashboardNotification('error', 'Failed to save: ' + error.message);
        } finally {
            submitBtn.disabled = false;
            submitBtn.innerHTML = '<i class="fas fa-save"></i> Save Links';
        }
    });
}